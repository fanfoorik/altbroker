...
export default class BrokerTable extends React.Component {
  ...
  getContacts = (event) => {
    ...
    ajax.post(`broker/gb/${id}/getcontacts/`).then((data) => {
      if (data.ERRORS.length) {
        this.showNotification((data.ERRORS[0].MESSAGE));
        return;
      }

      this.showNotification((data.ANSWER.MESSAGE));
    }).catch(() => {
      this.showNotification('Ошибка запроса');
            ...
            return (
              <div className={`table-row ${!editable ? ' not-editable' : ''}`} key={`table-item-${id}`}>
                ...
                <div className="table-cell table-col__broker no-padding-left">
                  {editable ? <TableBroker id={id} broker={broker} /> : broker }
                </div>
