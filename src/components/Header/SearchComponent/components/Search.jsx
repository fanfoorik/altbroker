...
export default class Notifications extends Component {
	...
	setCategory = (ev) => {
		...
		let activeOption = [].filter.call(this.categs, (item)=>{
		...
		if(activeOption[0] !== ev.target){
			...
			let category = ev.target.getAttribute("data-option");
				...
				<div className="search__content">
					...
					<div className="search__center clear">
						...
						<div className="search__text">
							<input className="search__input" value={this.state.value} onChange={this.setValue} type="text" placeholder="Что вы ищите?" ref="search__input" />
						</div>
					</div>
					...
					<div className="search-categs">
						<div className="search__center">
							...
							<span className="search-categs__option active" onClick={this.setCategory} data-option="broker">Брокер</span>
							<span className="search-categs__option" onClick={this.setCategory} data-option="deal">Сделки</span>
							<span className="search-categs__option" onClick={this.setCategory} data-option="analitics">Аналитика</span>
							<span className="search-categs__option" onClick={this.setCategory} data-option="progress">Развитие</span>
							<span className="search-categs__option" onClick={this.setCategory} data-option="management">Управление</span>
						</div>
					</div>
